...
printMemInfo()
{
	...
	if [ $? -eq 0 ]; then
		OUTPUT=`printf %s "$OUTPUT" | grep "Maximum Capacity" | sed -r 's/\s+Maximum Capacity: //'`
	else
		OUTPUT=`printf "%s KB (without kernel allocations)" "$(free | grep Mem: | awk '{print $2}')"`
	fi
...
removeBackupFiles()
{
	...
	ls $FILE_* 2>/dev/null | egrep '_[0-9]{8}_[0-9]{4}$' | sort -r | tail -n +$N | xargs rm -f > /dev/null
